...
SELECT date, duration, COUNT(loan_id) FROM loan WHERE date >=971201 AND date <=971231 GROUP BY date, duration ORDER BY date ASC;
...
SELECT account_id, type, SUM(amount) FROM trans WHERE account_id = 396 AND (type="PRIJEM" OR  type="VYDAJ") GROUP BY type; 