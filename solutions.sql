...
SELECT date, duration, COUNT(loan_id) FROM loan WHERE date LIKE '9712%' GROUP BY date, duration ORDER BY date ASC;
...
SELECT account_id, type, SUM(amount) as total_ammount FROM trans WHERE account_id = 396 AND (type="PRIJEM" OR  type="VYDAJ") GROUP BY type; 